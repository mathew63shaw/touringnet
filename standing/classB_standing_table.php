<div class="table-responsive">
        ...
        <table class="pointstable">
            ...
            <?php
            $i = 0;
            while ($i < count($classB_data)) {
                $driver_id = $classB_data[$i]['driver_id'];
                $exit_flag = false; ?>

                <tr class='alternate2'>
                    <td align='center'><?php echo $classB_data[$i]['rank']; ?></td>
                    <?php
                    if ($classB_cl_flag) { ?>
                        <td><?php echo $classB_data[$i]['class']; ?></td>
                    <?php } ?>
                    <td align='left'>
                        <img src="../results/flag/<?php echo $classB_data[$i]['image']; ?>.gif" title="<?php echo $classB_data[$i]['country']; ?>">&nbsp;<?php echo $classB_data[$i]['driver']; ?>
                    </td>
                    <td><?php echo $classB_data[$i]['points']; ?></td>

                    <?php
                    for ($j = 0; $j < count($column_header); $j++) {
                        $round = $column_header[$j]['round'];
                        // drove shared car in the current round
                        if (array_key_exists($round, $shared_classB[$driver_id])) { 
                            $lender_id = $shared_classB[$driver_id][$round];
                            $lender_race_result = $classB_race_result_array[$lender_id][$round][0];
                            $cls_name = $classB_race_result_array[$lender_id][$round][1];
                            $race_result = $classB_race_result_array[$driver_id][$round][0];

                            // drove his own car in the current round and lender's car
                            if (array_key_exists($round, $classB_race_result_array[$driver_id])) { ?>
                            ...
                            <?php }
                        } else if (array_key_exists($round, $classB_race_result_array[$driver_id])) { // drove his own car in the current round
                            $race_result = $classB_race_result_array[$driver_id][$round][0];
                            $cls_name = $classB_race_result_array[$driver_id][$round][1];
                            $exit_flag = true;
                            $i++; ?>
                            <td align='center' class='<?php echo $cls_name; ?>'>
