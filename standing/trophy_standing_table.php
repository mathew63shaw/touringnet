<div class="table-responsive">
    ...
    <table class="pointstable">
        ...
        <?php
        $i = 0;
        while ($i < count($trophy_data)) {
            $driver_id = $trophy_data[$i]['driver_id'];
            $exit_flag = false; ?>

            <tr class='alternate2'>
                <td align='center'><?php echo $trophy_data[$i]['rank']; ?></td>
                <?php
                if ($trophy_cl_flag) { ?>
                    <td><?php echo $trophy_data[$i]['class']; ?></td>
                <?php } ?>
                <td align='left'>
                    <img src="../results/flag/<?php echo $trophy_data[$i]['image']; ?>.gif" title="<?php echo $trophy_data[$i]['country']; ?>">&nbsp;<?php echo $trophy_data[$i]['driver']; ?>
                </td>
                <td><?php echo $trophy_data[$i]['points']; ?></td>

                <?php
                for ($j = 0; $j < count($column_header); $j++) {
                    $round = $column_header[$j]['round'];
                    // drove shared car in the current round
                    if (array_key_exists($round, $shared_trophy[$driver_id])) {
                        $lender_id = $shared_trophy[$driver_id][$round];
                        $lender_race_result = $trophy_race_result_array[$lender_id][$round][0];
                        $cls_name = $trophy_race_result_array[$lender_id][$round][1];
                        $race_result = $trophy_race_result_array[$driver_id][$round][0];

                        // drove his own car in the current round and lender's car
                        if (array_key_exists($round, $trophy_race_result_array[$driver_id])) { ?>
                        ...
                        <?php }
                    } else if (array_key_exists($round, $trophy_race_result_array[$driver_id])) { // drove his own car in the current round
                        $race_result = $trophy_race_result_array[$driver_id][$round][0];
                        $cls_name = $trophy_race_result_array[$driver_id][$round][1];
                        $exit_flag = true;
                        $i++; ?>
                        <td align='center' class='<?php echo $cls_name; ?>'>
