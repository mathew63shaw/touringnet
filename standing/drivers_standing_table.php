<div class="table-responsive">
        ...
        <table class="pointstable">
            ...
            <?php
            $i = 0;
            while ($i < count($drivers_data)) {
                $driver_id = $drivers_data[$i]['driver_id'];
                $exit_flag = false; ?>

                <tr class='alternate2'>
                    <td align='center'><?php echo $drivers_data[$i]['rank']; ?></td>
                    <?php
                    if ($drivers_cl_flag) { ?>
                        <td><?php echo $drivers_data[$i]['class']; ?></td>
                    <?php } ?>
                    <td align='left'>
                        <img src="../results/flag/<?php echo $drivers_data[$i]['image']; ?>.gif" title="<?php echo $drivers_data[$i]['country']; ?>">&nbsp;<?php echo $drivers_data[$i]['driver']; ?>
                    </td>
                    <td><?php echo $drivers_data[$i]['points']; ?></td>
                    ...
                    for ($j = 0; $j < count($column_header); $j++) {
                        ...
                        if (array_key_exists($round, $shared_drivers[$driver_id])) { 
                            ...
                            <?php }
                        } else if (array_key_exists($round, $drivers_race_result_array[$driver_id])) { // drove his own car in the current round
                            $race_result = $drivers_race_result_array[$driver_id][$round][0];
                            $cls_name = $drivers_race_result_array[$driver_id][$round][1];
                            $exit_flag = true;
                            $i++; ?>
                            <td align='center' class='<?php echo $cls_name; ?>'>
